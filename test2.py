...
def download_onnxruntime_binaries():
    if not os.path.exists(os.path.join(onnxruntime_genai_dir, "ort")):
        ...
        return
    print("File is already unpacked, skipping source downloads.")


def build_onnxruntime_genai(repo_dir):
    if not os.path.exists(os.path.join(repo_dir, "build")):
        os.chdir(repo_dir)
...
def install_wheel(repo_dir):
    os.chdir(repo_dir)
    wheel_dir = os.path.join("build", "Windows", "Release", "wheel")
...
def main():
    # If either onnxruntime or onnxruntime-genai is missing, perform the setup
    if not check_package("onnxruntime") or not check_package("onnxruntime-genai"):
        ...
        print("onnxruntime and onnxruntime-genai are installed.")
